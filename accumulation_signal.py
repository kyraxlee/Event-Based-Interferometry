...
#x_lower, x_upper = 50, 220
#y_lower, y_upper = 110, 310
x_upper, x_lower = 346, 0
y_upper, y_lower = 260, 0
...
for x, y, t ,p in focused_events :
    ...
    if int(t // timestep) == index: 
        ...
        total_events[int(t // timestep)] = np.sum((surface))
...
# Perform FFT on the total events
fft_result = np.fft.fft(total_events)
fft_freq = np.fft.fftfreq(len(total_events), d=timestep / 1_000_000)
magnitude = np.abs(fft_result)

# Plot the FFT result
plt.figure(figsize=(8, 4))
plt.plot(fft_freq[:len(fft_freq)//2], magnitude[:len(magnitude)//2], linestyle='-', color='r', label="FFT")
